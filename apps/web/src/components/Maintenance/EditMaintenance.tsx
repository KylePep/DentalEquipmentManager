...
import { updateMaintenanceEventAction } from "@/app/actions";
...
export function EditMaintenance({ maintenanceEvent, onSaved }: EditMaintenanceProps) {
  const [reoccurring, setReoccurring] = useState(maintenanceEvent.reoccur);
  ...
  async function handleSubmit(formData: FormData) {
    setPending(true);
    await updateMaintenanceEventAction(maintenanceEvent.id, formData);
    setPending(false);
    onSaved();
  }
  return (
    <form action={handleSubmit} className="flex flex-col gap-4 p-4 bg-stone-800 rounded max-w-xl">
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          type="text"
          maxLength={120}
          defaultValue={maintenanceEvent.title}
      ...
      <div className="flex flex-col gap-1">
        ...
        <textarea
          ...
          cols={50}
          maxLength={1200}
          defaultValue={maintenanceEvent.description ?? ""}
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          required
          maxLength={120}
          defaultValue={maintenanceEvent.start ?? null}
          className="bg-stone-900 text-white placeholder:text-gray-500 rounded px-1" />
      </div>
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          required
          maxLength={120}
          defaultValue={maintenanceEvent.end ?? ""}
          className="bg-stone-900 text-white placeholder:text-gray-500 rounded px-1" />
      </div>
      ...
      {reoccurring && (
        <div className="flex flex-col gap-1">
          ...
          <select defaultValue={maintenanceEvent.occurrence} name="occurrence" id="occurrence">
